...
class MenuMapperTest {
    ...
    void selectAllMenu() {
        ...
        assertThat(list)
                ...
                .allSatisfy(menu -> {
                    ...
                    assertThat(menu.getOrderableStatus()).satisfiesAnyOf( // statisfiesAnyOf : 최소한 하나를 만족하는지 검증하는 메소드
    ...
    void insertMenu() {
        ...
        // when
        int result = menuMapper.insertMenu(menu); // insert 후에 menu의 menuCode 필드에는 현재 생성된 메뉴번호가 담겨있을 것
        if(result > 0)
            sqlSession.commit();
